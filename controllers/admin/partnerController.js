...
async function editPartner(req, res) {
  try {
    ...
    // Admin session validation (same style as publisher)
    const session = await isAdminSessionValid(req, res);
    if (!session?.success) {
      return res.status(401).json({ success: false, msg: "Unauthorized" });
    }
    ...
    const beforePlain = partner.get({ plain: true });
    const {
      password: oldPass,
    ...
    // Uniqueness checks (only if username/email is being changed)
    ...
    const afterPlain = partner.get({ plain: true });
    const {
      password: newPass,
    ...
    // Decide if this was a soft-delete/restore change
    const isSoftDeleteOperation = typeof payload.is_deleted === "number";

    // Only log *real* edits (not is_deleted changes)
...
async function deletePartner(req, res) {
  try {
    // Validate :id (same style as deletePublisher)
    const schema = Joi.object({
      id: Joi.number().integer().positive().required(),
    });
    const { error, value } = schema.validate(req.params, {
    ...
    // Admin session validation (same as editPartner / editPublisher)
    const session = await isAdminSessionValid(req, res);
    if (!session || session.success !== true) {
      return res.status(401).json({ success: false, msg: "Unauthorized" });
    }
    ...
    // Before-delete snapshot (same pattern as deletePublisher)
    const beforePlain = partner.get({ plain: true });
    const {
      password: oldPass,
      two_fa_secret: oldSecret,
    ...
    // If already deleted, do not log again
    ...
    // Your existing "delete" logic → soft delete via is_deleted
    await partner.update({ is_deleted: 1 });

    // Admin log for delete (like deletePublisher)
    await logAdminAction({
      adminId,
      actionCategory: "partner",
      actionType: "DELETED",
      beforeData: beforeLog,
      afterData: null, // nothing after delete (we just indicate it's deleted)
    });
